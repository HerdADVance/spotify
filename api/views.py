...
# --------------------------------------
@redirect_if(no_spotify_token)
def get_show_episodes(request, data):

	# Number of episodes and show id from front end
	num_episodes = request.POST.get('num_episodes') or 10
	show = json.loads(request.POST.get('show'))
	...
	# Check our DB to see if user hasn't exceeded API call limits, return error if has
	ip_address = request.META.get('REMOTE_ADDR')
	allowed_to_send = check_api_access_limits(ip_address, 'show-episodes')
	if not allowed_to_send:
		return send_error("You have exceeded your hourly limit for searching a show's episodes. Please wait until later to try again.", 400)

	# Save new search attempt to DB or send error
	saved_attempt = save_search_attempt(ip_address, 'show-episodes')
	if not saved_attempt:
		return send_error('There was a problem with our database. Please try again later.', 400)
	...
	try:
		...
		return send_error('There was a problem retrieving episodes for this show. Please try again.', 400)

	# Loop through show episodes and append
	for episode in episodes_result:
		episodes.append({
			'name': episode['name'],
			'podcast_name': show['name'],
	...
	# Sort and format Episodes by date
	episodes = sorted(episodes, key=lambda x: x['release_date'], reverse=True)
	episodes = format_dates(episodes)
